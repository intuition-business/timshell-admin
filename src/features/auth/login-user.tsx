import { IconHeadset } from "@tabler/icons-react";
import Image from "next/image";

interface LoginProps {
    setIdentifier: string; 
    identifier: string; 
    error: string; 
    success: string; 
    isSubmitting: string; 
    handleSubmit: string;
}

export default function Login({ setIdentifier, identifier, error, success, isSubmitting, handleSubmit }: any) {
    return (
        <div className="h-screen w-full bg-[#0f0f0f] relative overflow-hidden">
            <div className="absolute -bottom-40 -left-40 w-[37.5rem] h-[37.5rem] rounded-full
                bg-[#D4FF00] opacity-20 blur-[11.25rem]"></div>
            <div className="absolute -top-40 -right-40 w-[37.5rem] h-[37.5rem] rounded-full
                bg-[#D4FF00] opacity-15 blur-[12.5rem]"></div>

            <div className="relative z-10 flex flex-col items-center justify-center h-full">
                <div className="relative z-10 flex items-center justify-center mt-8">
                    <Image
                        src={"/logo.png"}
                        alt="Timshel"
                        width={300}
                        height={150}
                        priority
                    />
                </div>
                <div className="my-auto -translate-y-[3.125rem] text-center">
                    <h2 className="text-[#D4FF00] text-[2rem] font-semibold mb-5">Iniciar sesión</h2>
                    <p className="text-white mb-4 text-lg">Ingresa tu teléfono y correo para recibir tu código de inicio de sesión</p>

                    <input
                        className="bg-[#2B2C2A] text-white border border-white px-6 py-4 rounded-xl w-full max-w-[31.25rem] text-lg my-3"
                        type="text"
                        placeholder="Ingresa tu Correo o Número de teléfono"
                        value={identifier}
                        onChange={(e) => {
                            let v = e.target.value.trim();
                            v = v.replace(/[^a-zA-Z0-9@._+\-()\s]/g, "");
                            setIdentifier(v);
                        }}
                    />

                    {error && <p className="text-red-500 mt-3">{error}</p>}
                    {success && <p className="text-green-500 mt-3">{success}</p>}

                    <button
                        className={`px-4 py-4 w-full rounded-xl font-semibold mt-4 max-w-[31.25rem] transition-all text-lg duration-200 
                        ${isSubmitting ? 'bg-gray-500 cursor-not-allowed' : 'bg-[#D4FF00] hover:bg-[#b9de00]'}`}
                        disabled={isSubmitting}
                        onClick={handleSubmit}
                    >
                        {isSubmitting ? 'Enviando...' : 'Confirmar'}
                    </button>
                    <a href="#" className="w-max mx-auto hover:text-gray-300 text-white text-lg flex gap-2 items-center justify-center mt-7"><IconHeadset></IconHeadset> <span>Llamar a soporte tecnico</span>
                    </a>
                </div>
            </div>
        </div>
    );
}
